...
const getCountry = (onSuccess, onError) => {  // foi inserido o parametro onError
  setTimeout(() => {
    const didOperationSucceed = Math.random() >= 0.5;
    if(didOperationSucceed) {
      const country = {
      ...
      const errorMessage = "Country could not be found";
      onError(errorMessage);  // foi inserido o parametro onError para ser ativada quando gerar algum erro
    }
  }, delay());
};
