...
function completeMonth(array) {
  for (let index = 0; index < dezDaysList.length; index += 1) {
    ...
    element.setAttribute('class', 'day'); //setAttribute utilizado para adicionar um atributo (classe) e o valor desejado para o atributo (day)
...
function holidays(holid) {
  for (let key of holid)  {
    for (let index2 = 0; index2 < document.getElementsByClassName('day').length; index2 += 1) {
      if (key.toString() === document.getElementsByClassName('day')[index2].innerText) {  //toString utilizado para converter um número em formato de string
...
function createFridays(buttFriday) {
  let buttonContainer2 = document.querySelector('.buttons-container');
  let newButton2 = document.createElement('button');
  let newButtonID2 = 'btn-friday';

  newButton2.innerText = buttFriday;
...
function displayFridays(fridaysArray) {
  ...
  getFridayButton.addEventListener('click', function() {
    for (let index2 = 0; index2 < getFridays.length; index2 += 1) {
      if (getFridays[index2].innerHTML !== newFridayText) {
        getFridays[index2].innerHTML = newFridayText;
      } else {
        getFridays[index2].innerHTML = fridaysArray[index];
      }
    }
  });
}
